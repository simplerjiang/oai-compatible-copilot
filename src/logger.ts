...
class Logger {
	 ...
	 */
	init(): void {
		this._logDir = path.join(os.homedir(), ".copilot", "oaicopilot-kong", "logs");
		this.reloadConfig();
		this.ensureLogDir();
	...
	reloadConfig(): void {
		...
		this._level = config.get<LogLevel>("oaicopilot-kong.logLevel", "off");
	}
	...
	private getLogFilePath(): string {
		...
		return path.join(this._logDir, `oaicopilot-kong-${dateStr}.log`);
	}
	...
	private write(level: string, tag: string, data: Record<string, unknown>): void {
		...
		try {
			...
			console.error("[OAICopilot-Kong Logger] Failed to write log:", e);
		}
	}
	...
	private cleanOldLogs(): void {
		try {
			...
			for (const file of files) {
				// Extract date from filename: oaicopilot-kong-YYYYMMDD.log
				const match = file.match(/^oaicopilot-kong-(\d{4})(\d{2})(\d{2})\.log$/);
				if (!match) {
					continue;
				}
			...
			console.error("[OAICopilot-Kong Logger] Failed to clean old logs:", e);
		}
	}
}
